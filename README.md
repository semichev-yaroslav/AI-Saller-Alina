...
Демонстрационная версия backend-приложения AI-менеджера продаж для Telegram.

`AI Saller Alina` автоматически обрабатывает входящие сообщения клиентов, определяет `intent`, стадию лида, генерирует ответ через LLM и сохраняет историю диалога. Персона бота: Алина, менеджер по продажам по внедрению AI в бизнес-процессы.
